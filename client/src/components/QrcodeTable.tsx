...
export default function QrcodeTable({
  ...
  return (
    <div className="bg-white rounded-xl shadow p-4 w-full flex flex-col">
      <h1 className="text-lg font-bold mb-2 text-gray-800">QR CODE GENERATED</h1>
          ...
          <tbody>
            {dataQrcode.map((qrcode) => (
              <tr key={qrcode.id} className="border-b border-gray-200">
                ...
                <td className="px-2 py-1">
                  <span className={`inline-block w-3 h-3 rounded-full mx-auto ${qrcode.status ? "bg-green-500" : "bg-gray-400"}`}></span>
                </td>
                <td className="px-2 py-1">
                  {qrcode.create_date ? new Date(qrcode.create_date).toLocaleString() : "-"}
                </td>
