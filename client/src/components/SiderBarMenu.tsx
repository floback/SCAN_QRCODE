...
export default function Sidebar({ isOpen = true, onClose }: SidebarProps) {
  ...
  return (
    <>
      <aside
        className={`fixed top-0 left-0 h-full z-40 bg-white border-r border-gray-200 shadow-md transition-all duration-300 ease-in-out
        ${isSidebarOpen ? "w-52 p-4" : "w-16 p-2"}
        md:rounded-r-2xl`}
      >
        <div className="flex flex-col h-full justify-between">
              ...
              <div className="flex items-center gap-3 px-2 py-2 mb-5 rounded-lg bg-gray-100">
                ...
                {isSidebarOpen && (
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-gray-600 capitalize">{user.type_user}</span>
                  </div>
                )}
              </div>
            )}
          ...
          <div className="mt-4 border-t border-gray-200 pt-4">
            <Link
              href="/auth"
              className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-600 hover:bg-red-100"
            >
              <LogOut size={18} />
